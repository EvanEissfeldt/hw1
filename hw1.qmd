...
2. Clone your Github repository on your local machine
    ...
    ```bash
    $ git clone <<https://github.com/EvanEissfeldt/hw1>>
    $ cd hw-1
    ```
...
```
```{R}
secondM <- matrix(c(1:100, (1:100)^2),
                  nrow = 2,
                  byrow = TRUE)
secondM
```
...
I expect col_wise_scan to take slightly longer because in the if statement it has to jump from column to column 
...
It seems that in each case, the row_wise_time takes longer. It also seems like there is a difference of 7 in a column for each of the tests, which is interesting. I do not understand why this difference of 7 appears in each test.
